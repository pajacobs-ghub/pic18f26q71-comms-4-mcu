...
// 2025-10-03 PIC18F26Q71 with different pin-assignments
...
void uart1_init(long baud)
{
    ...
    //
    // Configure PPS RX1=RC7, TX1=RC5, TX1DE=RC6 
    GIE = 0;
    PPSLOCK = 0x55;
    PPSLOCK = 0xaa;
    PPSLOCKED = 0;
    U1RXPPS = 0b010111; // RC7
    U1CTSPPS = 0b011011; // RD3 does not exist and should always read as 0
    RC5PPS = 0x15; // UART1 TX
    RC6PPS = 0x16; // UART1 TXDE
    // Do not assign UART1RTS to any output pin.
    PPSLOCK = 0x55;
    PPSLOCK = 0xaa;
    PPSLOCKED = 1;
    ANSELCbits.ANSELC5 = 0; // TX pin
    TRISCbits.TRISC5 = 0; // output
    ANSELCbits.ANSELC6 = 0; // TXDE pin
...
//----------------------------------------------------------------------------
// UART2 is used to communicate with Pico2 DAQ-MCU.
...
void uart2_init(long baud)
{
    ...
    unsigned int brg_value;
    //
    // Configure PPS RX2=RB2, TX2=RB0 
    GIE = 0;
    PPSLOCK = 0x55;
    PPSLOCK = 0xaa;
    PPSLOCKED = 0;
    U2RXPPS = 0b001010; // RB2
    RB0PPS = 0x18; // UART2 TX
    U2CTSPPS = 0b011010; // RD2 does not exist and should always read as 0
    PPSLOCK = 0x55;
    PPSLOCK = 0xaa;
    PPSLOCKED = 1;
    ANSELBbits.ANSELB0 = 0; // TX2 on RB4 pin
    TRISBbits.TRISB0 = 0; // output
    ANSELBbits.ANSELB2 = 0; // Turn on digital input buffer for RX2
    TRISBbits.TRISB2 = 1; // RX2 on RB5 is an input
    //
    U2CON0bits.BRGS = 1;
    brg_value = (unsigned int) (FOSC/baud/4 - 1);
